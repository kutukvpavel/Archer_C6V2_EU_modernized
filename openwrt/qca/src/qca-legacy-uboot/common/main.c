...
{	
	...
	len = nm_lib_readPtnFromNvram(FS_UBOOT_OFFSET,buf,bufLen);
#else
	len = nm_api_readPtnFromNvram(NM_PTN_NAME_FS_UBOOT, buf, bufLen);
#endif
	if (len < 0) return -1;

	printf("factory boot load fs uboot len %d to addr 0x%x.\n", len, TEXT_BASE);
