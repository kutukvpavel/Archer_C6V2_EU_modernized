...
void ath_gpio_config(void)
{
    ...
    // GPIO1  WPS(JumpStart) LED,    output, negative logic
    // GPIO2  WPS(JumpStart) Button, input,  negative logic
    // GPIO5  RESET Button,           input,  negative logic

	// GPIO6  Power LED,             output, negative logic
    // GPIO7  WAN LED1,               output, negative logic
    // GPIO8  WAN LED2,              output, negative logic
    // GPIO9  WIFI 5G LED,           output, negative logic
    // GPIO14 WIFI 2G LED,           output, negative logic
    // GPIO15 LAN LED,              output, negative logic
    // GPIO16  NOT USED,              output, negative logic

#if defined(CFG_UBOOT_C6V2_EU)
    // set GPIO6 and GPIO5 as input
    ath_reg_rmw_set(GPIO_OE_ADDRESS, (1 << 6) | (1 << 5));
    // set GPIO output
    ath_reg_rmw_clear(GPIO_OE_ADDRESS,  (1 << 1) | (1 << 7 ) | (1 << 8) |\
										(1 << 9) | (1 << 15) | (1 << 17) | \
										(1 << 19) | (1 << 20));



    // turn on  wan green! it's low at default!
	ath_reg_rmw_clear(GPIO_OUT_ADDRESS,(1 << 7));
	ath_reg_rmw_set(GPIO_OUT_ADDRESS,(1 << 8));

    // turn all LED on,but for GPIO19,it's turn off USB power.
     ath_reg_rmw_clear(GPIO_OUT_ADDRESS, (1 << 9 ) | (1 << 15 ) |  \
				 (1 << 17) | (1 << 19) | (1 << 20)); 
    udelay(500 * 1000);

    // turn on  wan orange ! it's low at default!
	ath_reg_rmw_clear(GPIO_OUT_ADDRESS,(1 << 8));
	ath_reg_rmw_set(GPIO_OUT_ADDRESS,(1 << 7));


	udelay(500 * 1000);

    // turn all LED off ,keep power on
    ath_reg_rmw_set(GPIO_OUT_ADDRESS,   (1 << 9 ) |  \
                                        (1 << 15) | (1 << 19) | \
                                        (1 << 20));

	ath_reg_rmw_clear(GPIO_OUT_ADDRESS,(1 << 7));
	ath_reg_rmw_clear(GPIO_OUT_ADDRESS,(1 << 8));   //turn off wan led

	ath_reg_rmw_clear(GPIO_OUT_ADDRESS,(1 << 17));  //turn on  power led
#else
    // set GPIO2 and GPIO5 as input
    ath_reg_rmw_set(GPIO_OE_ADDRESS, (1 << 2) | (1 << 5));
    // set GPIO output
    ...
    // turn all LED on,but for GPIO19,it's turn off USB power.
     ath_reg_rmw_clear(GPIO_OUT_ADDRESS, (1 << 1 ) | (1 << 6 ) |  \
				 (1 << 9) | (1 << 14) | \
                                        (1 << 15)); 
    udelay(500 * 1000);
