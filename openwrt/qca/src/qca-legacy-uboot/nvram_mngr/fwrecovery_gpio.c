...
#if defined(CONFIG_PRODUCT_C6V2)

#if defined(CFG_UBOOT_C6V2_EU)

static void c6v2SchemeInit(void)
{
	ath_reg_rmw_set(GPIO_OE_ADDRESS,  (1 << 5)); /* use FactoryResetButton(GPIO1) as input indicator */
	ath_reg_rmw_clear(GPIO_OE_ADDRESS,(1 << 9));/* use WPSLED(GPIO9) as output indicator */
}

static int c6v2KeyPressed(void)
{
    int ret = 0;
    ret = getKeyValueByGPIO(5);   /* c7v4/c7v5 use FactoryResetButton(GPIO5) as FW Recovery Button */
    if (ret < 0) return 0;        /* No Key Pressed Dectected. */
...
static void c6v2TurnLedOn(void)
{
    /* Clear GPIO 1 to turn on WPS LED. */
    ath_reg_rmw_clear(GPIO_OUT_ADDRESS, (1 << 9));
}

static void c6v2TurnLedOff(void)
{
    /* Set GPIO 1 to turn off WPS LED. */
...
static FW_RECOVERY_GPIO_SCHEME FWRecoveryC6V2Scheme =
{
    .schemeName   = "C6V2",
    .schemeInit   = NULL,
    .isKeyPressed = NULL,
...
#if defined(CONFIG_PRODUCT_C6V2)
	if (!strcmp(FWRecoveryC6V2Scheme.schemeName, FIRMWARE_RECOVERY_GPIO_SCHEME))
	{
		FWRecoveryC6V2Scheme.schemeInit = c6v2SchemeInit;
		FWRecoveryC6V2Scheme.isKeyPressed = c6v2KeyPressed;
		FWRecoveryC6V2Scheme.turnLedOn  = c6v2TurnLedOn;
		FWRecoveryC6V2Scheme.turnLedOff = c6v2TurnLedOff;
		FW_recovery_scheme = &FWRecoveryC6V2Scheme;
		return;
	}
#endif
