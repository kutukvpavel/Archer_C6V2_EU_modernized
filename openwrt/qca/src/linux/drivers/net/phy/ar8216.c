		      ...
		      struct switch_port_link *link)
{
	...
	if (link->aneg) {
		...
		//printk("[LEO][%d]port-link:%d",__LINE__,link->link);
		if (port == 0)
		{
			link->link = true;
		}	
		else
		{
			link->link = !!(status & AR8216_PORT_STATUS_LINK_UP);
			if (!link->link){
				//printk("[LEO][%d] port:%d,link:%d,status:%d\n",__LINE__,port,link->link,status);
				return;
				}	
		}		
	}
...
#else /*to fix bug 128345, switch lost packets when in heavy traffic test.*/
                ar8216_sw_reg_set(priv, 0x970, 0x11864200);
		ar8216_sw_reg_set(priv, 0x974, 0x000001c2);
		ar8216_sw_reg_set(priv, 0x978, 0x11008642);
		ar8216_sw_reg_set(priv, 0x97c, 0x000001c2);
		ar8216_sw_reg_set(priv, 0x980, 0x11008642);
		ar8216_sw_reg_set(priv, 0x984, 0x000001c2);
		ar8216_sw_reg_set(priv, 0x988, 0x11008642);
		ar8216_sw_reg_set(priv, 0x98c, 0x000001c2);
		ar8216_sw_reg_set(priv, 0x990, 0x11008642);
		ar8216_sw_reg_set(priv, 0x994, 0x000001c2);
		ar8216_sw_reg_set(priv, 0x998, 0x11864200);
		ar8216_sw_reg_set(priv, 0x99c, 0x000001c2);
		ar8216_sw_reg_set(priv, 0x9a0, 0x11864200);
		ar8216_sw_reg_set(priv, 0x9a4, 0x000001c2);
#endif
...
{
	struct ar8327_platform_data *pdata;
	struct ar8327_port_cfg *cfg = NULL;
	u32 t;
...
#define AR8337_GET_VLAN_CTRL0_CVID(_val) (((_val) & 0x0fff0000) >> 16)
#define WAN_IF_NAME_PREX  "eth0"
#define MAX_PORT_NUM	5

#include <linux/kmod.h>
#include <linux/fs.h>
#include <linux/uaccess.h>
static int write_wan_flag(int flag)
{
	static char buf_off[] = "0";
	static char buf_on[] = "1";
	struct file *fp;
	mm_segment_t fs;
	loff_t pos;
	fp = filp_open("/tmp/wan_connect_flag",  O_RDWR | O_CREAT | O_TRUNC, 0666);
	if(IS_ERR(fp))
	{
	...
	if(flag == 1)
	{
		vfs_write(fp,buf_on, sizeof(buf_on), &pos);
	}
	else
	{
		vfs_write(fp,buf_off, sizeof(buf_off), &pos);
	}
	filp_close(fp, NULL);
	set_fs(fs);
	return 0;
...
#ifdef CONFIG_LAN_LED_SINGLE
static void ar8216_led_lan_onoff(int status)
{
	char argv_buf[10] = {'\0'};

	char* cmd_path= "/usr/bin/ledcli";
	char* cmd_argv[3] = {cmd_path, argv_buf, NULL};
	char* cmd_envp[] = {"HOME=/", "PATH=/sbin:/bin:/usr/bin", NULL};
	sprintf(argv_buf, "LAN_%s",  status?"ON":"OFF");
	call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_WAIT_EXEC);
}
#endif
static void ar8216_led_onoff(int portid, int status)
{
	char argv_buf[10] = {'\0'};
	...
	printk("ar8216_led_onoff: port=%d, stautus=%d.\n", portid, status);
	
#ifdef CONFIG_PORTID_OVERTURN
	portid = 4 - portid;
#endif
	...
	if (portid == 0)
	{
		...
		if (status == 0)
		{
			sprintf(argv_buf, "WAN0_OFF");
			call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_WAIT_EXEC);
			sprintf(argv_buf, "WAN1_OFF");
			call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_WAIT_EXEC);	
			//printk("set wan connet flag to 0\n");
			write_wan_flag(0);
		}
		else
		{
			sprintf(argv_buf, "WAN0_ON");
			call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_WAIT_EXEC);
			sprintf(argv_buf, "WAN1_OFF");
			call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_WAIT_EXEC);
			//printk("set wan connet flag to 1\n");
			write_wan_flag(1);
		}
	}
#ifndef CONFIG_LAN_LED_SINGLE
	else
	{
		
		sprintf(argv_buf, "LAN%d_%s", portid, status?"ON":"OFF");
		call_usermodehelper(cmd_path, cmd_argv, cmd_envp, UMH_WAIT_EXEC);
	}
#endif
}

#define bit_is_set(a, bit) (((a) >> (bit)) & 0x1)
 /*   111100   LAN4~LAN1 WAN CPU*/
#define LAN_PORT_IDS  (0x3c)    
static int
ar8216_read_status(struct phy_device *phydev)
{
	...
	u32 vlan_val;
	int port_vlan_num = 0;
	int port_vlan_ctrl0[MAX_PORT_NUM] = {0x0428, 0x0430, 0x0438, 0x440, 0x448};
	int port_vlan_id[MAX_PORT_NUM] = {0};
	int port_vlan_bit[MAX_PORT_NUM] = {0};
	...
	if (priv->chip_ver == AR8XXX_VER_AR8337)
	{
		change = (priv->old_port_status ^ port_status);
		if (change)
		{
			printk("ar8216_read_status: %#x, %#x, %#x.\n", priv->old_port_status, port_status, change);
			for (i = 1; i <= MAX_PORT_NUM; i++)
			{
				if (bit_is_set(change, i))
				{
					ar8216_led_onoff((i - 1), bit_is_set(port_status, i));	
				}

			}
		#ifdef CONFIG_LAN_LED_SINGLE
			if( ((port_status & LAN_PORT_IDS) == 0) &&  (priv->old_port_status & LAN_PORT_IDS) )
			{
				ar8216_led_lan_onoff(0);
				printk("LAN_LED OFF\n");
			}
			else if( ((priv->old_port_status & LAN_PORT_IDS) == 0) &&  (port_status & LAN_PORT_IDS) )
			{
				ar8216_led_lan_onoff(1);
				printk("LAN_LED ON\n");
			}
		#endif
		}
		...
		{
			reg_val = priv->read(priv, port_vlan_ctrl0[i]);
			vlan_val = AR8337_GET_VLAN_CTRL0_CVID(reg_val);
			
			for (j = 0; j < port_vlan_num; j++)
			{
				if (vlan_val == port_vlan_id[j])
				{
					port_vlan_bit[j] |= 1 << (i+1);
					break;
				}
			}
			if (j == port_vlan_num)
			{
				port_vlan_id[j] = vlan_val;
				port_vlan_bit[j] |= 1 << (i+1);
				port_vlan_num++;
			}
		}

		for (i = 0; i < port_vlan_num; i ++)
		{
			sprintf(dev_name, "%s.%d", WAN_IF_NAME_PREX, port_vlan_id[i]);
			wan_dev = dev_get_by_name(&init_net, dev_name);
			if (wan_dev)
			{
				if ((port_status & port_vlan_bit[i]) != 0 && !netif_carrier_ok(wan_dev))
				{
					printk("%s_%d: %s carrier on!port_vlan_id = %d\n", __FUNCTION__, __LINE__, wan_dev->name, port_vlan_id[i]);
					netif_carrier_on(wan_dev);
				}
				else if ((port_status & port_vlan_bit[i]) == 0 && netif_carrier_ok(wan_dev))
				{
					
					//if(port_vlan_id[i] !=1 )
				#if defined(CONFIG_ATHRS17_HNAT) && defined(CONFIG_ATHRS17_HNAT_WIFI)
					if(!(wan_dev->priv_flags & IFF_HNAT_WIFI_REDIRECT_PORT))
				#endif
					{	//LAN VID is 1
						//ignore this operation for eth0.1 interface because this operation may 
						// break wifi HNAT
						
						printk("%s_%d: %s carrier off!port_vlan_id = %d\n", __FUNCTION__, __LINE__, wan_dev->name, port_vlan_id[i]);
	...
	if((priv->old_port_status ^ port_status) != 0) {
		for(i = 0; i < AR8X16_MAX_VLANS; i++) {
			if(((port_status & priv->vlan_table[i]) != 0) &&
				...
				if(ret == 1) {
					printk("%s_%d: %s carrier off!vlan_status = %d, vlan-table[%d] = %d\n", __FUNCTION__, __LINE__, priv->vlan_dev[i]->name, priv->vlan_status[i], i, priv->vlan_table[i] );
					sprintf(dev_name, "%s.%d\0", phydev->attached_dev->name, i);
					wan_dev= dev_get_by_name(&init_net, dev_name);
					if(wan_dev)
					{
					#if defined(CONFIG_ATHRS17_HNAT) && defined(CONFIG_ATHRS17_HNAT_WIFI)
