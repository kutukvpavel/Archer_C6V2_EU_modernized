...
static int ag71xx_rx_packets(struct ag71xx *ag, struct net_device *dev, int limit)
{
	...
	int received = 0;
	struct sk_buff *skb;
#if defined(CONFIG_ATHRS17_HEADER_EN)
	uint16_t athr_tag = 0xAAAA;
	uint8_t header_len = 0;
	uint8_t athr_tag_offset = ETH_ALEN * 2;
#endif
	bool has_ar8216;
...
#if defined(CONFIG_ATHRS17_HEADER_EN)
		if (*(uint16_t *)&skb->data[athr_tag_offset] == athr_tag) {
			header_len = 4;
			memcpy(skb->head, &skb->data[athr_tag_offset + 2], sizeof(aos_header_t));
			if (! ((((aos_header_t *)skb->head)->type == 0x1) || (((aos_header_t *)skb->head)->type == 0x2))) {	
				if (((aos_header_t *)skb->head)->with_tag) {
					((aos_header_t *)skb->head)->vid = 0xfff & *(uint32_t *)&skb->data[athr_tag_offset+header_len];
				} else {
					if ((*(uint32_t *)&skb->data[athr_tag_offset+header_len] & 0xFFFF0000) == 0x81000000) {
						((aos_header_t *)skb->head)->vid = 0xfff & *(uint32_t *)&skb->data[athr_tag_offset+header_len];
					} else {
						((aos_header_t *)skb->head)->vid = 0;
						printk("something is wrong, packet type is %x\n", ((aos_header_t *)skb->head)->type);
					}
				}
			}
		#if defined(CONFIG_ATHRS17_HNAT)
			if (((aos_header_t *)skb->head)->type == 0x10) {
				skb->is_hnat_frag = 1;
			}
		#ifdef CONFIG_ATHRS17_HNAT_WIFI
			if (!((aos_header_t *)skb->head)->sport ) {
				skb->is_wifi_redirect = 1;	// sport = 0, skb is wifi_redirect
			}	

		#endif
	
		#endif
			memmove(&skb->data[header_len], &skb->data[0], athr_tag_offset);
