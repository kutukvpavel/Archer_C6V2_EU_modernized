...
#define ROUTER_MAC_ENTRY_MAX 8
#define ARP_ENTRY_MAX 128

/* P6 is used by loop dev. */
#define S17_P6PAD_MODE_REG_VALUE 0x01000000

#define MULTIROUTE_WR 1

extern struct net init_net;
extern unsigned char hnat_wan_mac[ETH_ALEN];
extern unsigned char hnat_lan_mac[ETH_ALEN];
extern unsigned int hnat_lan_ip;
extern unsigned int hnat_lan_mask;

//char *nat_lan_dev_list = "eth0.1";
//char *nat_wan_dev_list = "eth0.4094";
char nat_lan_dev_list[128] = "eth0.1";
char nat_wan_dev_list[128] = "eth0.4094";

#define NAT_WAN_DEV_VID 4094


static int wan_fid = 0xffff;
static fal_pppoe_session_t pppoetbl = {0};
static uint32_t pppoe_gwid = ARP_ENTRY_MAX;
static char lan_nat_bridge_dev[IFNAMSIZ*4] = {0};
static char wan_nat_bridge_dev[IFNAMSIZ*4] = {0};
static uint8_t lanip[4] = {0}, lanmask[4] = {0}, wanip[4] = {0};
static uint16_t dev_vid_list[ROUTER_MAC_ENTRY_MAX] = {0};
static uint32_t dev_vid_index = 0;
static int32_t wifi_dev_vid = -1;
...
static void wan_nh_add(u_int8_t *host_ip , u_int8_t *host_mac, u_int32_t id)
{
    ...
    if (i < MAX_HOST)
    {
        ...
        if (host_mac != NULL)
        {
            ...
            {
                droute_add_acl_rules(*(uint32_t *)&lanip, *(uint32_t *)&lanmask, id);
                /* set the in_acl flag */
                wan_nh_ent[i].in_acl = 1;
            }
        }
            ...
            if (next_hop != 0)
            {
                ...
                else
                {
                    if (wan_nh_get(next_hop) != -1)
                        droute_add_acl_rules(*(uint32_t *)&lanip, *(uint32_t *)&lanmask, arp_entry.entry_id);
                    else
                        printk("%s %d\n", __FUNCTION__, __LINE__);
                }
            }
...
static sw_error_t setup_interface_entry(char *list_if, int is_wan)
{
    ...
    while ((dev_name = strsep(&list_all, " ")) != NULL)
    {
        ...
        if (NULL != br_port_get_rcu(nat_dev)) /* under bridge interface. */
        {
            ...
            if (! is_wan) {
		memcpy(lan_nat_bridge_dev, br_name, IFNAMSIZ);
	    } else {
		memcpy(wan_nat_bridge_dev, br_name, IFNAMSIZ);
	    }		
            /* Get dmac */
...
#endif
#ifdef ISISC
        if ((0 == is_wan) && !setup_private_ip) /* Not WAN -> LAN */
        {
            ...
            if ((in_device_lan) && (in_device_lan->ifa_list))
            {
               ...
               // memcpy(&lanip, (void *)&(in_device_lan->ifa_list->ifa_address), 4); /* copy Lan port IP. */
		        //memcpy(&lanmask, (void *)&(in_device_lan->ifa_list->ifa_mask), 4); /* copy LAN mask. */

                nat_hw_prv_mask_set((a_uint32_t)(hnat_lan_mask));
                nat_hw_prv_base_set((a_uint32_t)(hnat_lan_ip));
#ifndef ISISC
                ...
                redirect_internal_ip_packets_to_cpu_on_wan_add_acl_rules((a_uint32_t)(hnat_lan_ip),
															                   (a_uint32_t)(hnat_lan_mask));

#endif
                if (vlan_enable)
                {
                    tplink_change_vlan_priority(*(uint32_t *)&lanip, *(uint32_t *)&lanmask, vlan_prio, 1);
                }
            }
#ifdef CONFIG_ATHRS17_HNAT_WIFI
			if (wifi_dev_vid < 0)
			{
				nat_dev->priv_flags |= IFF_HNAT_WIFI_REDIRECT_PORT;
				wifi_dev_vid = vid;
				printk("wifi dev vid: %d %x\n", wifi_dev_vid, nat_dev->priv_flags);
			}
#endif
		    setup_private_ip = 1;	
        }
		...
		if (dev_vid_index < ROUTER_MAC_ENTRY_MAX) {
			dev_vid_list[dev_vid_index] = vid;
			dev_vid_index ++;
		} else {
			printk("no more entry for router mac\n");
			setup_error = SW_FAIL;
			break;
		}
        ...
        {
			tplink_redirect_invalid_ttl_to_cpu(devmac, is_wan);
            setup_error = SW_OK;
        }
    }
...
static int setup_all_interface_entry(void)
{
	...
	if (IP_INTF_ENTRY_IS_EMPTY(0) == SW_OK) {
		setup_lan_if = 0;
		setup_wan_if = 0;
		setup_default_vid = 0;
		memset(dev_vid_list, 0, ROUTER_MAC_ENTRY_MAX * 2);
		dev_vid_index = 0;
	}
...
static void isis_pppoe_check_for_redial(void)
{
            ...
            && (pppoetbl.session_id != 0))
    {
        if(pppoetbl.session_id != nf_athrs17_hnat_ppp_id)
        {
            ...
            pppoe_add_acl_rules(*(uint32_t *)&wanip, *(uint32_t *)&lanip, *(uint32_t *)&lanmask, pppoe_gwid);
        }
    }
}
...
static int add_pppoe_host_entry(uint32_t sport, a_int32_t arp_entry_id)
{
...
#endif
    {
        ...
        if (rv == SW_ALREADY_EXIST)
        {
            PPPOE_SESSION_TABLE_DEL(0, &pppoetbl);
            rv = PPPOE_SESSION_TABLE_ADD(0, &pppoetbl);
        }
        if (rv == SW_OK)
        {
            ...
            if (a_entry_id >= 0) /* hostentry creation okay */
            {
                ...
                memset(&nh_arp_entry,0,sizeof(nh_arp_entry));
				if(pppoe_gwid == ARP_ENTRY_MAX -1) 
				{
				    nh_arp_entry.entry_id = FAL_NEXT_ENTRY_FIRST_ID;
				}
				else 
				{
				    nh_arp_entry.entry_id = pppoe_gwid;
				}
				rv = IP_HOST_NEXT(0,FAL_IP_ENTRY_ID_EN,&nh_arp_entry);
				if (rv == SW_OK)
				{
                    IP_HOST_DEL(0,FAL_IP_ENTRY_IPADDR_EN,&nh_arp_entry);
				}
					
				
                aos_printk("adding ACLs \n");
                pppoe_gwid = a_entry_id;
                pppoe_add_acl_rules(*(uint32_t *)&wanip, *(uint32_t *)&lanip, *(uint32_t *)&lanmask, a_entry_id);
...
arp_check(struct sk_buff *skb)
{
	...
	memcpy(&sip, (uint8_t *)(arp + 1) + MAC_LEN, 4);
	memcpy(&dip, (uint8_t *)(arp + 1) + MAC_LEN * 2 + 4, 4);
	if ((! sip) || (sip == dip)) {
		// remove gratuitous ARP request
		return 0;
	}
    return 1;
}
...
int dev_match_wan_interface(struct net_device *dev) {
	return (dev->type == ARPHRD_PPP) || dev_check((char *)dev->name, (char *)wan_nat_bridge_dev) || dev_check((char *)dev->name, (char *)nat_wan_dev_list);	
}
#ifndef ISISC
...
static int check_lanwan_ip(a_uint32_t sip, uint8_t dev_is_lan)
{
    char temp[IFNAMSIZ*4] = {0};
    char *devname;
    char * list = temp;
    int ret = 0;
	if (0 == dev_is_lan)
	{
		memcpy(temp,nat_wan_dev_list,strlen(nat_wan_dev_list)+1);
	}
	else
	{
		memcpy(temp,nat_lan_dev_list,strlen(nat_lan_dev_list)+1);
	}
    ...
    if (devname != NULL)
    {
        struct in_device *in_device_wan = NULL;
        struct net_device *nat_dev = dev_get_by_name(&init_net, devname);
        ...
        {
            if (in_device_wan->ifa_list->ifa_address == sip)
                ret = 1;
...
arp_in_bg_handle(struct nat_helper_bg_msg *msg)
{
    ...
    if(((wan_nat_bridge_dev[0] != '\0') && dev_check((char *)in->name, (char *)wan_nat_bridge_dev)) || dev_check((char *)in->name, (char *)nat_wan_dev_list))
    {

    }
    else if (((lan_nat_bridge_dev[0] != '\0') && dev_check((char *)in->name, (char *)lan_nat_bridge_dev)) || dev_check((char *)in->name, (char *)nat_lan_dev_list))
    {
        dev_is_lan = 1;
    }
    else
...
#endif
#ifdef CONFIG_ATHRS17_HEADER_EN
	...
	for (i = 0; i < dev_vid_index; i ++) {
		if (dev_vid_list[i] == vid)
			break;
	}
	if (i >= dev_vid_index) {
		printk("Not support device with VID %d\n", vid);
		return 0;
	}
    ...
    /*if sip equals wan ip,we think it as an arp attack*/
    if (nf_athrs17_hnat_wan_type == NF_S17_WAN_TYPE_PPPOE)
    {
   		 
		if (*((uint32_t*)sip) == htonl(nf_athrs17_hnat_wan_ip))
			return 0;
    }
       if (check_lanwan_ip(*((uint32_t*)sip), 0) || check_lanwan_ip(*((uint32_t*)sip), 1))
            return  0;

	/* handle arp request and arp reply, to avoid arp lost */
	if ((ntohs(arp->ar_op) == ARPOP_REQUEST) &&  (ip_route_input_noref(skb, udip, uip, 0, skb->dev) == 0))
	{
		...
		if (rt->rt_type == RTN_LOCAL)
		{
			...
			if((arp_entry_id = arp_hw_add(sport, vid, sip, smac, 0)) < 0)
			{
				/* 
				 * ip_route_input will inc the refcnt
				 * so need to dec the refcnt
				 */
				//dst_release(&rt->dst);
				return 0;
			}
			//printk("Setup hostentry %02x-%02x-%02x-%02x-%02x-%02x success(request)\n", smac[0], smac[1], smac[2], smac[3], smac[4], smac[5]);
		}	
		/* 
		 * ip_route_input will inc the refcnt
		 * so need to dec the refcnt
	...
	else if ((ntohs(arp->ar_op) == ARPOP_REPLY))
	{
		...
		//printk("Setup hostentry %02x-%02x-%02x-%02x-%02x-%02x success(reply)\n", smac[0], smac[1], smac[2], smac[3], smac[4], smac[5]);
	}
	else
	{
		return 0;
    ...
    if ((0 == dev_is_lan) && (arp->ar_op == htons(ARPOP_REPLY)))
    {
        memcpy(&wanip, dip, 4);
#ifdef MULTIROUTE_WR
            ...
            if ((lanip[0] == wanip[0]) && (lanip[1] == wanip[1]))
            {
                if ((lanip[2] & 0xf0) == (wanip[2] & 0xf0))
                {
                    ...
                    if ( (wan_nh_ent[0].host_ip != 0))
                        ip_conflict_add_acl_rules(*(uint32_t *)&wanip, *(uint32_t *)&lanip, *(uint32_t *)&lanmask, wan_nh_ent[0].entry_id);
#endif
                    return 0;
                }
            }
...
#ifdef CONFIG_ATHRS17_HNAT
extern void (*athrs_hnat_delete_frag_napt_entry)(struct sk_buff *skb);

static struct nf_conn* hnat_get_ct_instance(struct sk_buff *skb)
{
	struct iphdr *iph;
	struct nf_conntrack_tuple_hash *h;
	struct nf_conn *ct = NULL;
	struct nf_conntrack_tuple tuple;
	uint32_t ihl, in_hnat = 0;
	...
	iph = ip_hdr(skb);
	ihl = iph->ihl * 4;
	if (((iph->protocol == IPPROTO_TCP) && (skb->len >= ihl + sizeof(struct tcphdr))) || \
		((iph->protocol == IPPROTO_UDP) && (skb->len > ihl + sizeof(struct udphdr)))) {
		if (nf_ct_get_tuplepr(skb, skb_network_offset(skb), PF_INET, &tuple)) {
			h = nf_conntrack_find_get(dev_net(skb->dev), NF_CT_DEFAULT_ZONE, &tuple);
			if (h) {
...
void hnat_delete_frag_napt_entry(struct sk_buff *skb)
{
	struct nf_conn *ct = NULL;

	ct = hnat_get_ct_instance(skb);
	if (ct && (ct->in_hnat == 1)) {	
		printk("udp fragment del napt=%x, result = %d\n", (a_uint32_t)ct, napt_ct_del_by_ctaddr((a_uint32_t)ct));
		ct->in_hnat = -1;		
	}
...
#ifdef CONFIG_ATHRS17_HNAT_WIFI
extern int (*athrs_hnat_wifi_redirect)(struct sk_buff *skb);
int hnat_check_wifi_redirect(struct sk_buff *skb)
{
	struct nf_conn *ct = NULL;
	uint32_t in_hnat = 0;
...
a_int32_t tplink_acl_scan_route_table(void)
{
	a_int32_t rv = 0;

	if (*(uint32_t *)&lanip != 0) {
		fib_route_scan(&init_net, *(uint32_t *)&lanip, *(uint32_t *)&lanmask, update_static_route);
		rv = commit_static_route();
	}
	return rv;
}
#endif

#endif // end CONFIG_ATHRS17_HNAT

/*host_entry should not contain wan_ip*/
void remove_wrong_arp()
{
	sw_error_t rv;
	fal_host_entry_t arp_entry = {0};
	if (nf_athrs17_hnat_wan_type != NF_S17_WAN_TYPE_PPPOE)
		return;
	if (nf_athrs17_hnat_wan_ip == 0)
		return;	
	arp_entry.ip4_addr = 	nf_athrs17_hnat_wan_ip;
	arp_entry.flags    =    FAL_IP_IP4_ADDR;
	if ((rv = IP_HOST_GET(0, FAL_IP_ENTRY_IPADDR_EN, &arp_entry)) == SW_OK){ 
		printk("remove wrong arp addr is %x \n",arp_entry.ip4_addr );
		IP_HOST_DEL(0, FAL_IP_ENTRY_IPADDR_EN, &arp_entry);
	} 
...
static unsigned int ipv6_bg_handle(struct nat_helper_bg_msg *msg)
{
    ...
    if(PROTO_ICMPV6 == iph6->nexthdr)
    {
        if(NEIGHBOUR_AD == icmp6->icmp6_type)
        {
            ...
            if ((NULL != in->ip6_ptr) && (! list_empty(&(((struct inet6_dev *)in->ip6_ptr)->addr_list))))
#else
            if (NULL != in->ip6_ptr)
#endif
            {
...
extern void set_switch_info(char*wan, char*lan);
...
void host_helper_init(void)
{
...
#ifndef CONFIG_ATHRS17_HEADER_EN
    for (i=1; i<6; i++)
    {
    ...
    napt_procfs_init();
    set_switch_info(nat_wan_dev_list,nat_lan_dev_list);
    memcpy(&lanip, (void *)&(hnat_lan_ip), 4); /* copy Lan port IP. */
    memcpy(&lanmask, (void *)&(hnat_lan_mask), 4);
	memset(wan_nh_ent, 0,MAX_HOST* sizeof(struct wan_next_hop) );

	printk("host_helper_init, wifi_dev_vid =%d, reset it\n",wifi_dev_vid);
	wifi_dev_vid = -1; /*reset wifi vid beacause iptv may change it , by wangjiang */
	nat_hw_prv_base_update_enable();
	printk("enable nat_hw_prv_base_update\n");
...
#ifdef CONFIG_ATHRS17_HNAT
	RCU_INIT_POINTER(athrs_hnat_delete_frag_napt_entry, hnat_delete_frag_napt_entry);
#ifdef CONFIG_ATHRS17_HNAT_WIFI
	RCU_INIT_POINTER(athrs_hnat_wifi_redirect, hnat_check_wifi_redirect);
#endif
#endif
}
...
{
#ifdef CONFIG_ATHRS17_HNAT
	RCU_INIT_POINTER(athrs_hnat_delete_frag_napt_entry, NULL);
#ifdef CONFIG_ATHRS17_HNAT_WIFI
	RCU_INIT_POINTER(athrs_hnat_wifi_redirect, NULL);
#endif
#endif 
	memset(lanip, 0, 4);
    memset(lanmask,0,4);
	memset(wanip, 0, 4);
	pppoe_gwid = ARP_ENTRY_MAX;
	memset(&pppoetbl,0,sizeof(fal_pppoe_session_t));
	
    napt_procfs_exit();
	printk("host_helper_exit, wifi_dev_vid =%d, reset it\n",wifi_dev_vid);
	wifi_dev_vid = -1; /*reset wifi vid beacause iptv may change it , by wangjiang */
	clear_aclrulemask();
