...
create_vap() {
	...
	local b c v n
	local cfg="!shortgi !wds TxBFCTL !countryie !uapsd=1 powersave !ant_ps_on ps_timeout \
	...
	#Used for Beacon/Probe Response TP IE config
	tpie_mac=":$hw_addr"
	tpie_mac=${tpie_mac//:/ 0x}
	random_suffix="0x78 0x59"
	...
	if [ "$guest" = "0" ]; then
		...
		[ -n "$bintval" ] && iwpriv "$vif" bintval "$bintval"
	fi
	config_iwpriv "$vif" "$cfg"	
	config_get mcast_rate "$vif" mcast_rate
	[ -n "$mcast_rate" ] && iwpriv "$vif" mcast_rate "${mcast_rate%%.*}"
	#Add TP IE in Beacon/Probe Response
	config_get onemesh_ie "$vif" onemesh_ie
	[ "$onemesh_ie" = "on" ] && {
		iwpriv "$vif" setiebuf 0x0 0x0 0x0 0x0 0x0 0x0 0x0 0x1c 0xdd 0x1a 0x0 0x1d 0x0f 0x10 0x1 0x7 0x0 $tpie_mac $tpie_mac $random_suffix 0x0 0x0 0x0 0x0 0x0 
		iwpriv "$vif" setiebuf 0x0 0x0 0x0 0x2 0x0 0x0 0x0 0x1c 0xdd 0x1a 0x0 0x1d 0x0f 0x10 0x1 0x7 0x0 $tpie_mac $tpie_mac $random_suffix 0x0 0x0 0x0 0x0 0x0 
	}
...
wifi_reload() {
	...
	wifi_radio "$1" "txchainmask rxchainmask AMPDU ampdudensity !AMSDU AMPDULim AMPDUFrames AMPDURxBsize !bcnburst:set_bcnburst=0 \
		...
		!led_enable promisc block_interbss aggr_burst set_pmf txbf_snd_int=100 mcast_echo obss_rssi_th obss_rx_rssi_th disablestats" \
		&>$STDOUT
