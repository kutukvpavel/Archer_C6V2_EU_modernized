...
scan_qcawifi() {
	...
	config_set "$device" vifs "${ap:+$ap }${wrap:+$wrap }${sta:+$sta }${adhoc:+$adhoc }${wds:+$wds }${monitor:+$monitor}"
}
...
enable_qcawifi() {
	...
	config_get obss_rssi_th "$device" obss_rssi_th
	[ -n "$obss_rssi_th" ] && iwpriv "$phy" obss_rssi_th "${obss_rssi_th}"

	config_get obss_rx_rssi_th "$device" obss_rx_rssi_th
...
post_qcawifi() {
	...
	case "${action}" in
		enable)
			local icm_enable qwrap_enable iso=""
