...
hostapd_set_bss_options() {
	...
	case "$enc" in
		*psk*)
			...
			[ "$psk_version" = "auto" ] && enc="psk-mixed/"
			[ "$psk_version" = "rsn" ] && enc="wpa-psk2/"
			[ "$psk_version" = "wpa" ] && enc="wpa-psk/"
			if [ "$psk_cipher" = "auto" ];then
				enc="$enc""tkip+aes"
			else
				enc="$enc""$psk_cipher"
			fi
		...
		*wpa*)
			...
			[ "$wpa_version" = "auto" ] && enc="mixed-wpa/"
			[ "$wpa_version" = "rsn" ] && enc="wpa2/"
			[ "$wpa_version" = "wpa" ] && enc="wpa/"
			if [ "$wpa_cipher" = "auto" ]; then
				enc="$enc""tkip+aes"
			else
				enc="$enc""$wpa_cipher"
			fi
	...
	[ -n "$wpa_group_rekey" ] && [ "$enc" != "none" ] && append "$var" "wpa_group_rekey=$wpa_group_rekey" "$N"
	[ -n "$wpa_strict_rekey" ] && append "$var" "wpa_strict_rekey=$wpa_strict_rekey" "$N"
	...
	band2g_support=0
	band5g_support=0
	for phydev in $(cd /sys/class/net && ls -d wifi* 2>$STDOUT); do
		config_get phyband "$phydev" band
		if [ "$phyband" = "2g" ]; then
			band2g_support=1
		fi
		if [ "$phyband" = "5g" ]; then
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		...
		if [ "$band2g_support" = "1" -a "$band5g_support" = "1" ]; then
