...
connect_process()
{
	...
	vlist_for_each_element(&interfaces, iface, node) 
	{
		...
		if ((!iface->connectable || !link) && (iface->proto_handler ))
		{
			if ((iface->state == IFS_SETUP || iface->state == IFS_UP) && strcmp(iface->name, CONNECT_IFACE_MOBILE) != 0)
					connect_ifdown(iface);

			if (strcmp(iface->name, CONNECT_IFACE_MOBILE) != 0 || !iface->connectable || (iface->conn_mode != IFCM_DEMAND && iface->conn_mode != IFCM_AUTO))
			{
		...
		if ((iface->connectable &&  link) && (iface->proto_handler ))
		{
			if ((iface->state == IFS_TEARDOWN || iface->state == IFS_DOWN) && strcmp(iface->name, CONNECT_IFACE_MOBILE) != 0)
					connect_ifup(iface);
		}
