...
netifd_start_process(const char **argv, char **env, struct netifd_process *proc)
{
	...
	if (!pid) {
		...
		sprintf(cmdbuf, "echo \"start process cmd = %s \" > /dev/console", argv[0]);
		system(cmdbuf);
	...
	proc->uloop.pid = pid;
		sprintf(cmdbuf, "echo \" fork pid = %d \" > /dev/console", pid);
		system(cmdbuf);
...
{
		char cmdbuf[200] = {0};
	sprintf(cmdbuf, "echo \"try to kill process %d \" > /dev/console", proc->uloop.pid);
	system(cmdbuf);
	if (!proc->uloop.pending)
		return;
	memset(cmdbuf,0,200);
	sprintf(cmdbuf, "echo \"kill process %d \" > /dev/console", proc->uloop.pid);
	system(cmdbuf);
