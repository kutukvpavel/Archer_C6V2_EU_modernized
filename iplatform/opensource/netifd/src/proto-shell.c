		    ...
		    enum interface_proto_cmd cmd, bool force)
{
	...
	if (cmd == PROTO_CMD_SETUP) {
		...
		sprintf(cmdbuf, "echo \"proto_shell_handler settimeout 1 iface =%s \" > /dev/console", 
		state->proto.iface->name);
		system(cmdbuf);
		...
		sprintf(cmdbuf, "echo \"proto_shell_handler settimeout 2 iface =%s \" > /dev/console", 
		state->proto.iface->name);
		system(cmdbuf);
			...
			struct netifd_process *task)
{
	...
	case S_SETUP_ABORT:
		...
		sprintf(cmdbuf, "echo \"proto_shell_task_finish cancle 1 iface =%s \" > /dev/console", 
		state->proto.iface->name);
		system(cmdbuf);
	...
	case S_TEARDOWN:
		...
		sprintf(cmdbuf, "echo \"proto_shell_task_finish cancle 2 iface =%s \" > /dev/console", 
		state->proto.iface->name);
		system(cmdbuf);
...
proto_shell_teardown_timeout_cb(struct uloop_timeout *timeout)
{
	struct proto_shell_state *state;
		char cmdbuf[200] = {0};
	sprintf(cmdbuf, "echo \"proto_shell_teardown_timeout_cb \" > /dev/console");
	system(cmdbuf);
...
proto_shell_free(struct interface_proto_state *proto)
{
	struct proto_shell_state *state;
		char cmdbuf[200] = {0};
	sprintf(cmdbuf, "echo \"proto_shell_free \" > /dev/console");
	system(cmdbuf);
