...
void 
interface_set_connectable(struct interface *iface, bool new_state)
{
	iface->connectable = new_state;
	switch (iface->conn_mode)
	{
	case IFCM_DEMAND:
		iface->conn_time.last = time(NULL);
		break;
	case IFCM_MANUAL:
	default:
...
interface_set_up(struct interface *iface)
{
	...
	char cmdbuf[200] = {0};
	sprintf(cmdbuf, "echo \"interface_set_up state =  %d \" > /dev/console", iface->state );
	system(cmdbuf);
	...
	memset(cmdbuf,0,200);
	sprintf(cmdbuf, "echo \"interface_set_up true up \" > /dev/console" );
	system(cmdbuf);
